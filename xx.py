...
def get_compare(filename1, filename2):
	a = getImgHash(filename1)
	# 图片地址自行替换
	b = getImgHash(filename2)
	compare_png = getMH(a, b)
	return compare_png


if __name__ == '__main__':
	# filename1 = './cropimg/one91.png'
	# filename2 = './cropimg/one115.png'
	# compare = get_compare(filename1, filename2)
	# print(compare)
	# x = 0
	# png_list = os.listdir('./cropimg')
	# for z in png_list[:]:
	#
	# 	file1 = './cropimg/%s' % z
	...
	# 				x += 1
	# 				if file2 != file1:
	# 					compare = get_compare(file1, file2)
	# 					if compare > 80:
	# 						os.remove(file2)
	# 						print(' -------------------------- del')
	# 					else:
